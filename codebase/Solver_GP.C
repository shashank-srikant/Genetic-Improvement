...
{Log_count64++;/*145*/} if(ps.size() == 0) 
{
{Log_count64++;/*147*/} return ok = false;
}
else if(ps.size() == 1) 
{
((value(ps[0]) == l_Undef) ? static_cast<void> (0) : __assert_fail ("value(ps[0]) == l_Undef", "../core/Solver.C", 151, __PRETTY_FUNCTION__));{Log_count64++;/*151*/}
...
{
((c.size() > 1) ? static_cast<void> (0) : __assert_fail ("c.size() > 1", "../core/Solver.C", 168, __PRETTY_FUNCTION__));{Log_count64++;/*168*/}
...
{
{Log_count64++;/*211*/} if(value(c[i]) == l_True) 
{
...
case polarity_user: {Log_count64++;/*276*/}sign = polarity[next];break;
case polarity_rnd: {Log_count64++;/*277*/}sign = irand(random_seed, 2);break;
...
{
out_learnt.shrink(out_learnt.size() - j);{Log_count64++;/*426*/}
}
tot_literals += out_learnt.size();{Log_count64++;/*428*/}
...
{
((reason[var(analyze_stack.last())] != __null) ? static_cast<void> (0) : __assert_fail ("reason[var(analyze_stack.last())] != __null", "../core/Solver.C", 468, __PRETTY_FUNCTION__));{Log_count64++;/*468*/}
...
{Log_count64++;/*513*/} if(decisionLevel() == 0) 
{
...
{
((level[x] > 0) ? static_cast<void> (0) : __assert_fail ("level[x] > 0", "../core/Solver.C", 527, __PRETTY_FUNCTION__));{Log_count64++;/*527*/}
out_conflict.push(~trail[i]);{Log_count64++;/*528*/}
}
else
{
Clause& c = *reason[x];
...
{
c[0] = c[1], c[1] = false_lit;{Log_count64++;/*594*/}
}
...
{
nof_learnts *= learntsize_inc;{Log_count64++;/*685*/}
}
}
...
removeSatisfied(learnts);{Log_count64++;/*731*/}
{Log_count64++;/*732*/} if(remove_satisfied) 
{
removeSatisfied(clauses);{Log_count64++;/*734*/}
}
order_heap.filter(VarFilter(*this));{Log_count64++;/*738*/}
simpDB_assigns = nAssigns();{Log_count64++;/*740*/}
...
{
{Log_count64++;/*823*/} if(backtrackLevels[i]< LM) 
{
LM= backtrackLevels[i];{Log_count64++;/*825*/}
nofLM= 1;{Log_count64++;/*826*/}
}
...
{
progress_estimate = progressEstimate(); {Log_count64++;/*848*/}
...
{Log_count64++;/*871*/} if(value(p) == l_True) 
{
...
restartTolerance= nVars() / 10000 +10;{Log_count64++;/*948*/}
backtrackLevels= new int[restartMore];{Log_count64++;/*949*/}
lbool status = l_Undef;
while(status == l_Undef) 
{
status = search((int)nof_conflicts, (int)nof_learnts);{Log_count64++;/*965*/}
nof_conflicts *= restart_inc;{Log_count64++;/*966*/}
...
verifyModel();{Log_count64++;/*983*/}
}
else
{
((status == l_False) ? static_cast<void> (0) : __assert_fail ("status == l_False", "../core/Solver.C", 988, __PRETTY_FUNCTION__));{Log_count64++;/*988*/}
...
{Log_count64++;/*1043*/} if((int)clauses_literals != cnt) 
{
...
for(i = 1;i < sz;i++) 
{
{Log_count64++;/*1058*/} if((seen[var(out_learnt[i])] & (1|2)) == (1|2)) 
{
...
out_learnt[j++] = out_learnt[i];{Log_count64++;/*1061*/}
}
}
...
{
((maykeep == 0) ? static_cast<void> (0) : __assert_fail ("maykeep == 0", "../core/Solver.C", 1105, __PRETTY_FUNCTION__));{Log_count64++;/*1105*/}
...
{
Lit q = rp[i];
{Log_count64++;/*1173*/} if(level[var(q)] <= 0) 
{
