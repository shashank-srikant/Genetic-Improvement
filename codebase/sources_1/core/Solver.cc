/* syntax.awk Revision: 1.73  1  <IF_Solver_173><IF_Solver_690> <_Solver_561>   Thu Jul 13 11:04:03 BST 2017*/
/*../Solver.bnf_GP pop.001 ../Solver.bnf_GP*/
#include "Solver_H.h"
...
{
assert(decisionLevel() == 0);{Log_count64++;/*73*/}
...
assert(c.size() > 1);{Log_count64++;/*113*/}
...
{
const Clause& c = ca[cr];
assert(c.size() > 1);{Log_count64++;/*123*/}
...
{Log_count64++;/*173*/} if(verbosity >= 1) 
{
...
do
{
assert(confl != CRef_Undef);{Log_count64++;/*218*/}
Clause& c = ca[confl];
...
{
assert(reason(var(analyze_stack.last())) != CRef_Undef);{Log_count64++;/*327*/}
...
{
assert(level(x) > 0);{Log_count64++;/*371*/}
...
{
assert(value(p) == l_Undef);{Log_count64++;/*393*/}
...
assert(c[1] == false_lit);{Log_count64++;/*424*/}
i++;{Log_count64++;/*425*/}
Lit first = c[0];
...
{
assert(decisionLevel() == 0);{Log_count64++;/*523*/}
...
{
assert(value(assumptions[i]) != l_False);{Log_count64++;/*773*/}
